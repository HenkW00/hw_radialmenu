...
-------------BELOW IS FOR PLAYER ACTIONS MENU-----------------

local initialized = false  -- Flag to track initialization status

function sendToMedSystem(menu, item)
    print(menu, item)
    
    if menu == 'player_menu' and item == 1 then
        print('Test print')
    end
end
        ...
        lib.registerRadial({
            ...
            items = {
                ...
                },
                {
                    ...
                    icon = 'medkit',
                    onSelect = function()
                        exports['hw_medsystem']:SendMedicalRequest('emergency')
                        print("Medical emergency request sent")
                    end
                }
            }
        })
...
function point:onEnter()
    initializeRadialMenu()  -- Ensure radial menu is initialized when entering the point
    lib.addRadialItem({
        id = 'garage_access',
        icon = 'warehouse',
        label = 'Garage',
